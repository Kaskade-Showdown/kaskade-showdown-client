...
Storage.bg = {
	...
	load: function (bgUrl, bgid, hues) {
		...
		if (!hues) {
			switch (bgid) {
			case 'horizon':
				hues = ["353.33333333333337,32.43243243243244%", "319.09090909090907,34.920634920634924%", "266.8085106382979,22.48803827751196%", "217.05882352941174,47.22222222222222%", "222.65060240963857,33.60323886639676%", "197.6842105263158,54.913294797687875%", "232.00000000000003,19.480519480519483%", "228,65.65656565656568%"];
				...
				break;
			case 'ocean':
				hues = ["198.3206106870229,52.610441767068274%", "216.9230769230769,40.94488188976379%", "84.32432432432434,36.27450980392157%", "228.81355932203388,35.757575757575744%", "207.95454545454544,66.66666666666664%", "208.141592920354,56.218905472636806%", "213.38709677419357,57.4074074074074%", "199.2857142857143,48.275862068965495%"];
				...
				break;
			case 'shaymin':
				hues = ["15.000000000000016,5.660377358490567%", "340.42105263157896,41.85022026431718%", "33.06122448979592,26.77595628415302%", "157.77777777777777,12.79620853080568%", "154.2857142857143,2.766798418972328%", "170.00000000000003,12.5%", "175.00000000000003,13.043478260869565%", "44.210526315789515,19.999999999999993%"];
				attrib = '<a href="http://cargocollective.com/bluep" target="_blank" class="subtle">"Shaymin" <small>background by Daniel Kong</small></a>';
				break;
			case 'charizards':
				hues = ["186.42857142857144,50.000000000000014%", "73.58490566037737,58.241758241758255%", "191.84713375796179,81.34715025906736%", "11.180124223602485,71.55555555555554%", "178.56,52.30125523012552%", "22.191780821917806,37.43589743589743%", "36.84782608695652,75.40983606557377%", "211.03448275862067,28.155339805825246%"];
				...
				break;
			case 'psday':
				hues = ["344.1891891891892,61.15702479338842%", "206.66666666666666,26.359832635983267%", "300,18.875502008032132%", "260.6106870229008,60.368663594470064%", "20,20.547945205479454%", "165.9574468085106,46.07843137254901%", "254.02597402597405,48.42767295597485%", "24.705882352941195,41.46341463414633%"];
			...
			case 'digimon':
				...
				break;
			case 'kaskademap':
				hues = ["150,13.043478260869582%", "197.99999999999997,46.05263157894737%", "31.395348837209305,46.23655913978495%", "68.29787234042556,48.958333333333336%", "34.871794871794876,51.091703056768544%", "201.99999999999997,24.590163934426222%", "54.159292035398224,57.360406091370564%", "61.93548387096774,35.63218390804598%"];
		...
		img.onload = function () {
			...
			try {
				...
				if (!colors) {
					...
					var extractedColors = colors.length ? colors : [[0, 0, 0]];
					for (var i = 0; i < Storage.bg.MENU_BUTTONS; i++) {
						var color = extractedColors[Math.min(i, extractedColors.length - 1)];
...
try {
	...
	if (bg.length >= 2) {
		if (!Storage.bg.load(bg[0], bg[1]) && bg.length < 10) {
			Storage.bg.extractMenuColors(Storage.bg.curUrl || bg[0], Storage.bg.curId || bg[1], true);
		}
		if (bg.length >= 10) Storage.bg.loadHues(bg.slice(2));
	}
} catch (e) {}
