...
<h1>June 2013 attack on Kaskade Showdown</h1>

<div style="margin-left: 30px; font-style: italic;">
Published June 20, 2013.
</div>

<p>Yesterday, an attacker compromised Kaskade Showdown in order to acquire user passwords. He may have obtained up to 160,000 passwords in total (out of a total of 302,000 users).</p>

<p>You should immediately change your Kaskade Showdown password and, more importantly, you should change this password everywhere else it was used, such as your email account. If you don't change your passwords, the attacker may compromise your other accounts. If you've logged into Kaskade Showdown since May, your passwords are probably safe &mdash; but not necessarily &mdash; and you should change them anyway as a precaution.</p>
...
<p>As a result, anybody who logged into Kaskade Showdown after April 25, 2013 had their password re-hashed into the new secure format; however, people who have not logged into Kaskade Showdown since before April 25 have their passwords stored in the insecure format.</p>

<p>If an attacker were able to view the Kaskade Showdown database (and had no greater access), he would only be able to get the passwords of users whose passwords continued to be stored in the old format (and weak passwords stored in the new format). He would not be able to get the passwords of most users who have logged in since April 25, 2013, since those passwords are stored in a more secure format. However, using other kinds of attacks it would still be possible to obtain some other passwords.</p>

<h2>Details of the attack</h2>

<p>In this section, we will describe the specific attack perpetrated against Kaskade Showdown.</p>

<p>On the morning of June 19, 2013, an attacker compromised the Filecharger website, which is a different website, unrelated to Kaskade Showdown, which is owned by Zarel, the creator of Kaskade Showdown. Filecharger was written many years ago, and likely contained one or more vulnerabilities. Compromising Filecharger allowed the attacker to obtain Filecharger's user database, which contained passwords hashed using MD5, which as you now know, is insecure. Unfortunately, Zarel used the same password for Filecharger as he did on Kaskade Showdown.</p>

<p>Immediately after compromising Filecharger, at around 9:37 AM CDT on June 19, 2013, the attacker set his sights on Kaskade Showdown.</p>

<p>The attacker started by using an <a target="_blank" href="https://github.com/Zarel/Pokemon-Showdown-Client/commit/5fe36c3090ff9fb9149fb4819985499dd9516990">SQL injection vulnerability</a> in the ladder code to retrieve Kaskade Showdown passwords. This vulnerability, which we have now fixed, allowed <i>viewing</i> the database, but did not allow modifying it. As a result, the attacker would not have been able to obtain the password of any users who logged in since April 25, 2013 using this method alone, unless their passwords were notably weak. (However, many of those users may have had alts which had not logged in recently, and the attacker could obtain the alts' passwords, which might be the same as the main nick's passwords.)</p>

<p>At 12:06 PM CDT, the attacker successfully logged into Zarel's Kaskade Showdown account. He may have used Zarel's password obtained from Filecharger, or he may have reversed the password hash of one of Zarel's alts on Kaskade Showdown (which had not been used in months and so had insecure passwords). In any case, the attacker did not appear to make use of any software exploit to log in as Zarel, other than knowing Zarel's password.</p>

<p>After logging in as Zarel, the attacker accessed the admincp on the Kaskade Showdown forums and enabled a feature of phpBB3 that allows execution of PHP code in templates. The attacker then modified the FAQ template to insert a backdoor. He then used that backdoor to upload a PHP <a target="_blank" href="https://en.wikipedia.org/wiki/Shell_%28computing%29">shell</a> to the server. A shell is a tool for navigating the computer, kind of like the file manager on Windows, but more powerful. Using his shell, the attacker backed up a number of Kaskade Showdown config files for his own uses, and then modified our login code to log users' plaintext passwords to a log file (<code>/tmp/log.txt</code>).</p>
...
<p>Kaskade Showdown does not collect email addresses, so we cannot warn users by email. However, we will attempt to disseminate this information as widely as we can in order to protect users to the fullest extent possible.</p>
...
<p>Thanks for playing on Kaskade Showdown!</p>
